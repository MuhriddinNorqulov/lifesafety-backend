...
class SaveResultAPIView(APIView):
    ...
    def post(self, request, exam_item_id):
        ...
        elif len(data) > questions.count():
            raise ValidationError('')

        student = self.request.user
        if exam_item.group_id != student.group_id:
            raise NotFound()
        attempt_list = [i.attempt for i in StudentResult.objects.filter(student=student, exam_item=exam_item)]
        max_attempt = 0
        ...
        serializer = QuestionVariantIDSerializer(data=data, many=True, context={'exam_item_id': exam_item_id})
